...
d1FileSystem::d1FileSystem() : d1Module()
{
...
// Destructor
d1FileSystem::~d1FileSystem()
{
	PHYSFS_deinit();
}

// Called before render is available
bool d1FileSystem::Awake(pugi::xml_node& config)
{
...
bool d1FileSystem::AddPath(const char* path_or_zip, const char* mount_point)
{
	bool ret = false;
...
bool d1FileSystem::Exists(const char* file) const
{
	return PHYSFS_exists(file) != 0;
}

// Check if a file is a directory
bool d1FileSystem::IsDirectory(const char* file) const
{
	return PHYSFS_isDirectory(file) != 0;
}

// Read a whole file and put it in a new buffer
unsigned int d1FileSystem::Load(const char* file, char** buffer) const
{
	unsigned int ret = 0;
...
// Read a whole file and put it in a new buffer
SDL_RWops* d1FileSystem::Load(const char* file) const
{
	char* buffer;
...
unsigned int d1FileSystem::Save(const char* file, const char* buffer, unsigned int size) const
{
	unsigned int ret = 0;
