...
void UIButton::Draw()
{
	if (GetFrom() != nullptr)
	{
		...
		case UIHUDPANELBUTTONS:
			App->render->Blit((SDL_Texture*)App->uimanager->GetAtlas(), rect_position.x - 2 - App->render->camera->GetPosition().x, rect_position.y - 2 - App->render->camera->GetPosition().y, &mark_btn, SDL_FLIP_NONE, 0, 0, 1.0f, 0.0, false);
